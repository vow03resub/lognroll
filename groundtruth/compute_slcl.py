...
#                # <<1>> INFO org.mortbay.log: jetty-6.1.26
#                # <<2>> INFO org.mortbay.log: jetty.*..*..*
#                # <<3>> INFO org.mortbay.log: jetty..*...*...*
#                # <<4>> INFO org.mortbay.log: jetty..*...*...*
#                # INFO org.mortbay.log: .*
#                if item['type']=="int" or item['type']=="float":
#                    if m.start()>0:
#                        #print "\033[34;43m",item,"\033[0m"
#                        #print "\033[34;103m",log_template,"\033[0m"
#                        #print "\033[1;31m Pattern="+m.group(0),"    m.start()="+str(m.start()),"    m.end()="+str(m.end()),"\033[0m"
#                        #print "\033[0;46m",log_template[m.start()-diff-1],"\033[0m"
...
def detect_and_convert_list_object(input_logs):

    list_serial = 1
    processed = []

    for j in range(0,len(input_logs)):
        ...
        for i,c in enumerate(log):
            ...
            if c=='[':
                ...
                #print "} found", i, cnt
                if cnt==0:
                    sublog = log[jstart:jend+1]
                    if "," in sublog:
                        #print "Object found:", re.sub("[0-9]","1",sublog)
                        coord.append((jstart,jend))

        for tup in reversed(coord):
            newlog = log[:tup[0]]+"__LIST_OBJECT_"+format(list_serial,'05d')+"__"+log[tup[1]+1:]
            list_serial += 1
            log = newlog
...
def detect_and_convert_json_object(input_logs):
        ...
        for i,c in enumerate(log):
            ...
            if c=='{':
                ...
                if cnt==0:
                    sublog = log[jstart:jend+1]
                    if "," in sublog:
                        if "=" in sublog or ":" in sublog:
                            #print "Object found:", re.sub("[0-9]","1",sublog)
                            coord.append((jstart,jend))

        for tup in reversed(coord):
            newlog = log[:tup[0]]+"__JSON_OBJECT_"+format(json_serial,'05d')+"__"+log[tup[1]+1:]
            json_serial += 1
            log = newlog
...
if __name__ == '__main__':
    ...
    log_templates  = pickle.load(open("TMP1029348.bin","rb"))

    selected=[]
    mylogs = copy.deepcopy(raw_logs)
    xlog_templates=[] 
    log_mask=[-1 for _ in range(len(mylogs))] 
    for i in range(0,len(log_templates)):
        t = log_templates[i]
        if t==None: 
            continue
        ...
        progress_mod=max(5,len(log_templates)/40)
        if i%progress_mod==0:
            sys.stdout.write('\r'+"\033[0;103mMatch counting processed "+"{0:.1f}".format(float(i*100)/float(len(log_templates)))+"% \033[0m")
            sys.stdout.flush()
    ...
    print sum_matched, len(mylogs), len(log_templates), len(selected), SL, CPL
